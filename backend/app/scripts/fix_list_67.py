"""
Fix list 67 - create Trakt list and sync items.
"""
import sys
import asyncio
...
async def fix_list_67():
    """Create Trakt list for list 67 and sync items."""
    db = SessionLocal()
    try:
        # Get list 67
        user_list = db.query(UserList).filter(UserList.id == 67).first()
        if not user_list:
            logger.error("List 67 not found")
            return
        ...
        # Count items
        items = db.query(ListItem).filter(ListItem.smartlist_id == 67).all()
        ...
        if not user_list.trakt_list_id:
            ...
            # Update database
            db.query(UserList).filter(UserList.id == 67).update({
        ...
        if items and trakt_list_id:
            ...
            logger.info(f"✓ Added {sync_stats.get('added', 0)} items to Trakt")
