...
Run inside container:
  python /app/app/scripts/report_sync_metrics.py
"""
...
def _matches_filters(item: Dict[str, Any], filters: Dict[str, Any]) -> bool:
    ...
    if mr is not None:
        rt = item.get('rating')
        if rt is None or float(rt) < float(mr):
            return False
    return True
...
async def main():
    ...
    try:
        ...
        for ul in lists:
            ...
            tmdb_pct = (with_tmdb / cand_count) if cand_count else 0.0
            genres_pct = (with_genres / cand_count) if cand_count else 0.0
            lang_pct = (with_lang / cand_count) if cand_count else 0.0
            print(f"  Candidates: {cand_count} | metadata: TMDB {tmdb_pct:.0%}, genres {genres_pct:.0%}, lang {lang_pct:.0%}")
